...
class ImportExerciseDb extends Command
{
    ...
    protected $description = 'Import exercises from local JSON files (storage/app/exdb/*.json)';

    public function handle()
    {
        $base = storage_path('app/exdb');
        ...
        if ($batch) $this->flushExercises($batch);
    ...
    private function flushExercises(array $rows): void
    {
        DB::table('exercises')->upsert(
            ...
            ['source','source_id'],
            [
                'name','primary_muscle','equipment','image_url',
                'tags','body_parts','target_muscles','secondary_muscles',
                'equipments_j','instructions','keywords','updated_at'
            ]
        );
    }
